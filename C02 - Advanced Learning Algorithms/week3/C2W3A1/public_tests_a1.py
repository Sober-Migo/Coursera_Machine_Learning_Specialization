...
def model_test(target, classes, input_size):
    ...
    # Keras 3 Compatibility: Safely extract input shape
    actual_input_shape = list(target.input_shape) if hasattr(target, 'input_shape') else list(target.input.shape)
    assert actual_input_shape == [None, input_size], \
        f"Wrong input shape. Expected [None,  {input_size}] but got {actual_input_shape}"
    ...
    for layer in target.layers:
        ...
        # Keras 3 Compatibility: Safely extract output shape
    ...
    # Keras 3 Compatibility: Safely extract learning rate
